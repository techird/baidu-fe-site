<?php 
    ...
    try{
        ...
        shuffle($setsData);
        $setData = $setsData[0];
        // $random = array_rand($setsData, 1);        
        // $randomSet = $setsData[$random];
        $photos = getPhotosInList(array(
            'per_page' => '200',
            'set_id' => $setData['id']
        ));
    }catch(Exception $e){
        echo "<h1 style=\"font-size:30px; color:red;\">". $e->getMessage(). "</h1>";
        echo "<p>获取信息出错，请稍候重试</p>";
        exit;
    }
    ...
    <body>
        <script>
            app.Dispatcher.postMessage( "showPhoto", {
                total: "<?=$setData['photos']?>",
                setId: "<?=$setData['id']?>",
                primary: "<?=$setData['primary']?>"
            } );
        </script>
    </body>
</html>